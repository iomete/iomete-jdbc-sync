...
class DataSyncer:
    ...
    def run(self):
        logger.info("Data sync started...", source_connection=str(self.source_connection))
    ...
    def __migrate_table(self, table_name: str, sync_mode: SyncMode):
        logger.info("Syncing table", table=table_name, sync_mode=sync_mode)
        ...
        if self.drop_proxy_table_after_migration:
            logger.info("Cleaning up proxy table:", proxy_table_name=proxy_table_name)
            self.lakehouse.execute(f"DROP TABLE {proxy_table_name}")

        logger.info("Data sync completed for table: ", table_name=table_name)
